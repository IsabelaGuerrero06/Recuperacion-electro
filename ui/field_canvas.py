...
from ui.wave_canvas import _dibujar_atomo_estatico, _en_resonancia
...
class FieldCanvas(QWidget):
    ...
    Cuando ωr ≈ ωl (resonancia), la zona posterior al átomo
    no se dibuja — la onda es absorbida.
    """
    ...
    # ----------------------------------------------------------
    # Indicador de absorción (igual que en wave_canvas)
    # ----------------------------------------------------------
    ...
    def _paint_absorcion(self, painter, x_atomo_px, width, height):
        ...
        painter.drawText(x_atomo_px + 8, height // 2 - 4, "ABSORBIDO")
    ...
    def paintEvent(self, event):
        ...
        if self.wave_mode == "incidente_resultante":
            ...
            # Campo resultante — solo si NO hay resonancia
            if not resonancia:
            ...
            if self.wave_mode == "emitida":
                ...
                if self.emitida_func is None:
                    def field_fn(x, t):
                        return (self.resultante_func(x, t, self.atomo)
            ...
            # En resonancia: modos de zona derecha no dibujan nada
            skip_right = resonancia and self.wave_mode in ("resultante", "emitida")
            ...
            if not skip_right:
                ...
                # Modo incidente en resonancia: solo hasta el átomo
